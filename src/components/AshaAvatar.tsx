...
export default function AshaAvatar({ pose = "idle", mouthOpen, level, className }: { pose?: GuidePose; mouthOpen?: boolean; level?: number; className?: string }) {
  return (
    <span className={cn("relative inline-flex shrink-0 overflow-hidden rounded-full bg-gradient-to-br from-brand-100 to-accent-100 ring-1 ring-brand-200", className)}>
      <Asha pose={pose} mouthOpen={mouthOpen} level={level} face className="h-full w-full" />
    </span>
  );
}
